    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="font-['Syne'] font-bold text-2xl sm:text-3xl text-[var(--text-primary)]">JWT解析器</h1>
        </div>
        <p className="text-[var(--text-secondary)] ml-[52px]">粘贴JWT字符串，自动解码Header、Payload、Signature</p>
      </motion.div>

      {/* Input */}
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="mb-6">
        <label className="text-sm font-medium text-[var(--text-secondary)] mb-2 ml-1 block">JWT字符串</label>
        <textarea
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder="粘贴JWT Token..."
          aria-label="JWT字符串"
          className="tool-area w-full h-[120px] p-5 text-sm leading-relaxed resize-none outline-none focus:border-[#e94560]/30 transition-colors placeholder:text-[var(--text-faint)] font-mono"
        />
      </motion.div>
      ...
      {parsed && !parsed.error && (
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="space-y-6">
          ...
          <div className="glass-card p-6">
            ...
            <div className="bg-[var(--bg-hover)] rounded-lg p-4 font-mono text-sm text-[var(--text-secondary)] overflow-x-auto">
              <pre>{JSON.stringify(parsed.header, null, 2)}</pre>
            </div>
            <div className="mt-3 text-xs text-[var(--text-faint)]">
              <span>原始Base64: </span>
              <code className="font-mono bg-[var(--bg-hover)] px-2 py-1 rounded">{parsed.headerRaw}</code>
            </div>
          </div>
          ...
          <div className="glass-card p-6">
            ...
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
              {parsed.payload.iat && (
                <div className="bg-[var(--bg-hover)] rounded px-2 py-1">
                  <span className="text-[var(--text-faint)]">iat (签发时间): </span>
                  <span className="text-[var(--text-secondary)]">{new Date(parsed.payload.iat * 1000).toLocaleString()}</span>
                </div>
              )}
              {parsed.payload.exp && (
                <div className="bg-[var(--bg-hover)] rounded px-2 py-1">
                  <span className="text-[var(--text-faint)]">exp (过期时间): </span>
                  <span className="text-[var(--text-secondary)]">{new Date(parsed.payload.exp * 1000).toLocaleString()}</span>
                </div>
              )}
              {parsed.payload.nbf && (
                <div className="bg-[var(--bg-hover)] rounded px-2 py-1">
                  <span className="text-[var(--text-faint)]">nbf (生效时间): </span>
                  <span className="text-[var(--text-secondary)]">{new Date(parsed.payload.nbf * 1000).toLocaleString()}</span>
                </div>
              )}
            </div>
            <div className="mt-3 text-xs text-[var(--text-faint)]">
              <span>原始Base64: </span>
              <code className="font-mono bg-[var(--bg-hover)] px-2 py-1 rounded">{parsed.payloadRaw}</code>
            </div>
          </div>
