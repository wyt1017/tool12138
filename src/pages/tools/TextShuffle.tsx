    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="font-['Syne'] font-bold text-2xl sm:text-3xl text-[var(--text-primary)]">文本打乱</h1>
        </div>
        <p className="text-[var(--text-secondary)] ml-[52px]">多行文本随机打乱、抽奖抽取、加权随机等工具</p>
      </motion.div>
      ...
      >
        <div className="flex flex-wrap items-center gap-3">
          <span className="text-sm text-[var(--text-secondary)] mr-2">模式：</span>
          <button
            onClick={() => setMode('shuffle')}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              mode === 'shuffle' ? 'bg-[#ffd369]/15 text-[#ffd369]' : 'bg-[var(--bg-hover)] text-[var(--text-faint)] hover:text-[var(--text-secondary)]'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              mode === 'lottery' ? 'bg-[#ffd369]/15 text-[#ffd369]' : 'bg-[var(--bg-hover)] text-[var(--text-faint)] hover:text-[var(--text-secondary)]'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              mode === 'weighted' ? 'bg-[#ffd369]/15 text-[#ffd369]' : 'bg-[var(--bg-hover)] text-[var(--text-faint)] hover:text-[var(--text-secondary)]'
            }`}
          >
          ...
          {mode === 'shuffle' && (
            <div className="flex items-center gap-2">
              <label className="text-xs text-[var(--text-secondary)]">打乱次数</label>
              <input
                type="number"
                min={1}
                max={100}
                ...
                aria-label="打乱次数"
                className="w-20 px-3 py-1.5 rounded-lg bg-[var(--bg-hover)] border border-[var(--border-color)] text-[var(--text-primary)] text-sm outline-none focus:border-[#ffd369]/50"
              />
            </div>
          )}
          ...
          {(mode === 'lottery' || mode === 'weighted') && (
            <div className="flex items-center gap-2">
              <label className="text-xs text-[var(--text-secondary)]">抽取数量</label>
              <input
                type="number"
                min={1}
                value={lotteryCount}
      ...
      {input && (
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[var(--text-faint)]">总行数</span>
            <span className="font-mono text-sm text-[var(--text-secondary)]">{originalLines.length}</span>
          </div>
          {hasShuffled && (
            <>
              <div className="h-4 w-px bg-[var(--bg-secondary)]" />
              <div className="flex items-center gap-2">
                <span className="text-xs text-[var(--text-faint)]">结果行数</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
          ...
          <textarea
            ...
            aria-label="原始文本"
            className="tool-area w-full h-[360px] p-5 text-sm leading-relaxed resize-none outline-none focus:border-[#ffd369]/30 transition-colors placeholder:text-[var(--text-faint)]"
          />
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
          {mode === 'weighted' ? (
            <>
              <div className="flex items-center justify-between mb-2 ml-1">
                <label className="text-sm font-medium text-[var(--text-secondary)]">权重设置（每行对应一个权重值）</label>
              ...
              <textarea
                ...
                aria-label="权重设置"
                className="tool-area w-full h-[220px] p-5 text-sm leading-relaxed resize-none outline-none focus:border-[#ffd369]/30 transition-colors placeholder:text-[var(--text-faint)]"
              />
              <div className="tool-area w-full h-[128px] p-5 mt-2 overflow-y-auto">
                {hasShuffled && shuffledLines.length > 0 ? (
                  <ul className="space-y-1.5">
                    {shuffledLines.map((line, index) => (
                      <li key={index} className="flex items-center gap-3 text-sm text-[var(--text-secondary)] py-1 px-2 rounded hover:bg-[var(--bg-hover)] transition-colors">
                        <span className="text-xs text-[var(--text-faint)] w-8 text-right font-mono shrink-0">{index + 1}.</span>
                        <span>{line}</span>
                      </li>
                    ))}
                  </ul>
                ) : (
                  <p className="text-[var(--text-faint)] text-sm">点击下方按钮开始加权抽取...</p>
                )}
              </div>
            </>
          ) : (
            <>
              ...
              <div className="tool-area w-full h-[360px] p-5 overflow-y-auto">
                {hasShuffled && shuffledLines.length > 0 ? (
                  ...
                  <p className="text-[var(--text-faint)] text-sm">点击下方按钮开始{mode === 'lottery' ? '抽取' : '打乱'}...</p>
                )}
              </div>
            </>
          )}
      ...
      >
        ...
        {hasShuffled && (
          <button onClick={handleReset} className="btn-secondary !text-[var(--danger)] flex items-center gap-2">
