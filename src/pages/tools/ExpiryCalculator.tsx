    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="font-['Syne'] font-bold text-2xl sm:text-3xl text-[var(--text-primary)]">保质期计算器</h1>
        </div>
        <p className="text-[var(--text-secondary)] ml-[52px]">计算产品过期日期，管理多条产品记录</p>
      </motion.div>
        ...
        <div className="space-y-3">
          {products.map((product, index) => (
            <motion.div
              ...
              className="grid grid-cols-12 gap-3 items-end bg-[var(--bg-hover)] rounded-xl p-4 border border-[var(--border-color)] hover:border-[var(--border-color)] transition-colors"
            >
              {/* Product Name */}
              <div className="col-span-12 sm:col-span-3">
                <label className="text-xs text-[var(--text-faint)] block mb-1.5">产品名称</label>
                <input
                  type="text"
                  value={product.name}
                  ...
                  aria-label="产品名称"
                  className="w-full px-3 py-2 rounded-lg bg-[var(--bg-hover)] border border-[var(--border-color)] text-[var(--text-primary)] text-sm outline-none focus:border-[#e94560]/50 placeholder:text-[var(--text-faint)]"
                />
              </div>

              {/* Production Date */}
              <div className="col-span-6 sm:col-span-2">
                <label className="text-xs text-[var(--text-faint)] block mb-1.5">生产日期</label>
                <input
                  type="date"
                  ...
                  max={today}
                  aria-label="生产日期"
                  className="w-full px-3 py-2 rounded-lg bg-[var(--bg-hover)] border border-[var(--border-color)] text-[var(--text-primary)] text-sm outline-none focus:border-[#e94560]/50 [&::-webkit-calendar-picker-indicator]:invert"
                />
              </div>

              {/* Shelf Life Amount */}
              <div className="col-span-3 sm:col-span-2">
                <label className="text-xs text-[var(--text-faint)] block mb-1.5">保质期</label>
                <input
                  type="number"
                  min={1}
                  ...
                  aria-label="保质期"
                  className="w-full px-3 py-2 rounded-lg bg-[var(--bg-hover)] border border-[var(--border-color)] text-[var(--text-primary)] text-sm outline-none focus:border-[#e94560]/50"
                />
              </div>

              {/* Unit Selector */}
              <div className="col-span-6 sm:col-span-2">
                <label className="text-xs text-[var(--text-faint)] block mb-1.5">单位</label>
                <select
                  value={product.shelfUnit}
                  onChange={(e) => updateProduct(product.id, 'shelfUnit', e.target.value)}
                  aria-label="保质期单位"
                  className="w-full px-3 py-2 rounded-lg bg-[var(--bg-hover)] border border-[var(--border-color)] text-[var(--text-primary)] text-sm outline-none focus:border-[#e94560]/50 appearance-none cursor-pointer"
                >
                  <option value="day" className="bg-[var(--bg-elevated)]">天</option>
                  <option value="week" className="bg-[var(--bg-elevated)]">周</option>
                  <option value="month" className="bg-[var(--bg-elevated)]">月</option>
                  <option value="year" className="bg-[var(--bg-elevated)]">年</option>
                </select>
              </div>
              ...
              <div className="col-span-12 sm:col-span-3 flex justify-end">
                {products.length > 1 && (
                  <button onClick={() => removeProduct(product.id)} className="btn-secondary !py-2 !px-3 text-[var(--danger)]">
                    <Trash2 size={14} />
                  </button>
                )}
              </div>
        ...
        <>
          ...
          {soonestExpiring && soonestExpiring.status !== 'fresh' && (
            ...
            >
              ...
              <div>
                <span className="font-semibold text-[var(--text-primary)]">{soonestExpiring.name || `产品`}</span>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-[var(--text-faint)]">共 {processedProducts.length} 条记录</span>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h3 className="font-semibold text-[var(--text-primary)] text-base">{product.name || `产品 #${product.id}`}</h3>
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-[var(--text-faint)]">生产日期</span>
                        <span className="text-[var(--text-secondary)] font-mono">{formatDisplayDate(product.productionDate)}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-[var(--text-faint)]">保质期</span>
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-[var(--text-faint)]">过期日期</span>
                        <span className="text-[var(--text-primary)] font-medium font-mono">{formatDisplayDate(product.expiryDate)}</span>
                      </div>
      ...
      {processedProducts.length === 0 && products.some((p) => p.name || p.productionDate !== today) && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card p-8 text-center">
          <p className="text-[var(--text-faint)] text-sm">请完善产品信息以计算过期时间</p>
        </motion.div>
      )}
    </div>
  );
