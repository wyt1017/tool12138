  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val.endsWith(' ') || val.endsWith('\n')) {
      ...
      if (newTypedIndex >= words.length) {
        ...
        // 用局部变量计算准确率，避免 stats 旧值竞态
        const finalAccuracy = stats ? Math.round((stats.charsCorrect / stats.charsTyped) * 100) : 100;
        const finalStats: Stats = { wpm, accuracy: finalAccuracy, charsTyped: stats?.charsTyped ?? newTypedIndex, charsCorrect: stats?.charsCorrect ?? newTypedIndex, time };
        setStats(finalStats);
    ...
    <div className="max-w-3xl mx-auto px-6 py-12" onClick={handleClick}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="font-['Syne'] font-bold text-2xl sm:text-3xl text-white">打字速度测试</h1>
        </div>
        <p className="text-[#a8b2c1] ml-[52px]">测试你的英文打字速度和准确率，记录历史成绩</p>
      </motion.div>
        ...
        {[15, 25, 50, 100].map((n) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${
              wordCount === n ? 'text-white' : 'bg-white/5 text-[#666] hover:text-white'
            }`}
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-card p-8 mb-6">
            ...
            {words.map((word, i) => {
              ...
              if (i === typedIndex) {
                ...
                for (const ch of currentInput) {
                  visibleChars.push(
                    <span key={charIdx} className={ch === word[charIdx] ? 'text-white' : 'text-red-400 underline'}>{ch}</span>
                  );
                  charIdx++;
                }
                return (
                  <span key={i}>
                    {visibleChars}
                    <span className="text-[#a8b2c1]">{word.slice(charIdx)}</span>
                    <span className="inline-block w-0.5 h-6 bg-[#a78bfa] animate-pulse ml-0.5 align-middle" />
                  </span>
                );
              }
              return <span key={i} className="text-[#444]">{word} </span>;
            })}
          </div>
          ...
          <div className="flex items-center justify-between text-sm text-[#666]">
            <div className="flex items-center gap-4">
              {phase === 'typing' && (
                <>
                  <span>已输入：<b className="text-white">{typedIndex + 1}</b> / {words.length}</span>
                  {stats && <span>准确率：<b className="text-white">{stats.accuracy}%</b></span>}
                </>
              )}
            </div>
        ...
        <>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 }} className="glass-card p-8 mb-6">
            <div className="text-center mb-8">
              <div className="font-['Syne'] font-bold text-7xl text-white mb-2">{stats?.wpm ?? 0}</div>
              <div className="text-lg text-[#a8b2c1]">WPM（每分钟单词数）</div>
            </div>
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-white/5 rounded-xl p-4 text-center">
                <div className="text-2xl font-bold text-white mb-1">{stats?.accuracy ?? 0}%</div>
                <div className="text-xs text-[#666]">准确率</div>
              </div>
              <div className="bg-white/5 rounded-xl p-4 text-center">
                <div className="text-2xl font-bold text-white mb-1">{stats?.time ?? 0}s</div>
                <div className="text-xs text-[#666]">用时</div>
              </div>
              <div className="bg-white/5 rounded-xl p-4 text-center">
                <div className="text-2xl font-bold text-white mb-1">{wordCount}词</div>
                <div className="text-xs text-[#666]">测试长度</div>
              </div>
            </div>
          </motion.div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="glass-card p-5">
              ...
              <div className="space-y-2">
                {history.slice(0, 5).map((h, i) => (
                  <div key={i} className="flex items-center justify-between text-sm bg-white/5 rounded-lg px-4 py-2.5">
                    <span className="text-[#666]">{h.date}</span>
                    <span className="text-white font-semibold">{h.wpm} WPM</span>
                    <span className="text-[#666]">{h.accuracy}% 准确率</span>
                    <span className="text-[#444] text-xs">{h.length}词</span>
                  </div>
                ))}
              </div>
            </motion.div>
