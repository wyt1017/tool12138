    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="font-['Syne'] font-bold text-2xl sm:text-3xl text-[var(--text-primary)]">文本排序</h1>
        </div>
        <p className="text-[var(--text-secondary)] ml-[52px]">多维度文本排序：字母、长度、数值、随机打乱</p>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="flex items-center gap-6 mb-6">
        <label className="flex items-center gap-2 cursor-pointer select-none">
          ...
          <span className="text-sm text-[var(--text-secondary)]">去重空行</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
          <div className="flex items-center justify-between mb-2">
            <label className="text-sm font-medium text-[var(--text-secondary)] ml-1">输入文本</label>
            <span className="text-xs text-[var(--text-faint)]">{originalCount} 行</span>
          </div>
          <textarea
            value={input}
            ...
            aria-label="输入文本"
            className="tool-area w-full h-[360px] p-4 text-[var(--text-primary)] text-sm leading-relaxed resize-none outline-none focus:border-[#38bdf8]/30 transition-colors placeholder:text-[var(--text-faint)]"
          />
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.25 }}>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs text-[var(--text-faint)]">{sortedLines.length} 行</span>
          ...
          <div className={`tool-area w-full h-[360px] p-4 overflow-auto ${!sortedText ? 'flex items-center justify-center' : ''}`}>
            {sortedText ? (
              <pre className="text-[var(--text-primary)] text-sm leading-relaxed whitespace-pre-wrap break-all font-mono">{sortedText}</pre>
            ) : (
              <span className="text-[var(--text-faint)] text-sm">排序结果将在此显示...</span>
            )}
          </div>
        </motion.div>
      </div>
      ...
      {input && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-card p-4 mt-6 flex flex-wrap items-center gap-6 text-sm">
          <span className="text-[var(--text-faint)]">
            原始 <strong className="text-[var(--text-primary)]">{originalCount}</strong> 行
            <ArrowUpDown size={14} className="inline mx-2 text-[#38bdf8]" />
            排序后 <strong className="text-[var(--text-primary)]">{sortedLines.length}</strong> 行
