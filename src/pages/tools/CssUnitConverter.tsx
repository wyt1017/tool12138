...
export default function CssUnitConverter() {
  ...
  const convert = () => {
    ...
    // 基准为 0 时除以 0 得到 Infinity，需拦截
    if (baseFontSize <= 0 || parentFontSize <= 0) {
      setResults(null);
      return;
    }
    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="font-['Syne'] font-bold text-2xl sm:text-3xl text-[var(--text-primary)]">CSS单位转换</h1>
        </div>
        <p className="text-[var(--text-secondary)] ml-[52px]">px ↔ rem ↔ em ↔ % 相互转换</p>
      </motion.div>

      {/* Base Settings */}
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="glass-card p-6 mb-6">
        <h2 className="text-sm font-medium text-[var(--text-secondary)] mb-4">基准设置</h2>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-[var(--text-faint)] block mb-2">根元素字体大小（用于rem）</label>
            <div className="flex items-center gap-2">
              <input
                ...
                aria-label="根元素字体大小"
                className="bg-[var(--bg-hover)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm text-[var(--text-primary)] outline-none focus:border-[#f472b6]/30 w-full"
              />
              <span className="text-[var(--text-faint)]">px</span>
            </div>
          </div>
          <div>
            <label className="text-xs text-[var(--text-faint)] block mb-2">父元素字体大小（用于em/%）</label>
            <div className="flex items-center gap-2">
              <input
                type="number"
      ...
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-card p-6 mb-6">
        <h2 className="text-sm font-medium text-[var(--text-secondary)] mb-4">输入值</h2>
        <div className="flex items-center gap-4">
          <input
            ...
            placeholder="输入数值"
            aria-label="输入数值"
            className="bg-[var(--bg-hover)] border border-[var(--border-color)] rounded-lg px-4 py-3 text-lg text-[var(--text-primary)] outline-none focus:border-[#f472b6]/30 flex-1"
          />
          <select
            value={inputUnit}
            onChange={(e) => setInputUnit(e.target.value as 'px' | 'rem' | 'em' | '%')}
            aria-label="输入单位"
            className="bg-[var(--bg-hover)] border border-[var(--border-color)] rounded-lg px-4 py-3 text-lg text-[var(--text-primary)] outline-none focus:border-[#f472b6]/30 cursor-pointer"
          >
            <option value="px" className="bg-[var(--bg-elevated)] text-[var(--text-primary)]">px</option>
            <option value="rem" className="bg-[var(--bg-elevated)] text-[var(--text-primary)]">rem</option>
            <option value="em" className="bg-[var(--bg-elevated)] text-[var(--text-primary)]">em</option>
            <option value="%" className="bg-[var(--bg-elevated)] text-[var(--text-primary)]">%</option>
          </select>
      ...
      {results && (
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-card p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {['px', 'rem', 'em', '%'].map((unit) => (
              <div key={unit} className="bg-[var(--bg-hover)] rounded-lg p-4">
                <div className="text-xs text-[var(--text-faint)] mb-2">{unit.toUpperCase()}</div>
          ...
          <div className="mt-6 p-4 bg-[var(--bg-hover)] rounded-lg">
            <h3 className="text-xs text-[var(--text-faint)] mb-2">转换公式</h3>
